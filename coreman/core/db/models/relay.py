...
class RelayServer(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("host", "clawrelay_port", name="uq_relay_servers_host"),
    )
    ...
    clawrelay_port: Mapped[int] = mapped_column(Integer)
    agent_port: Mapped[int | None] = mapped_column(Integer)
    ssh_user: Mapped[str | None] = mapped_column(Text)
    runtime_env: Mapped[str] = mapped_column(Text, server_default=text("'host'"))
    ...
    def relay_url(self) -> str:
        if self.runtime_node_id:
            return f"http://{self.runtime_node_id}.runtime/{self.model_provider}"
        return f"http://{self.host}:{self.clawrelay_port}"
