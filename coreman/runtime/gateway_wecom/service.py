...
class GatewayWecomService(Service):
    ...
    @property
    def _draining(self) -> set[uuid.UUID]:
        """正在排空中的 bot：它们暂时「持有租约却没有 runner」，租约扫描必须放着不管。"""
        return self.lease_loop.busy
    ...
    async def drain_bot(self, bot_id: uuid.UUID) -> None:
        ...
        """
        await self.lease_loop.drain(bot_id)

    async def lease_drain(self, bot_id: uuid.UUID) -> None:
        """排空钩子。先把 runner 摘出 `self.runners`，推送 / 出站两条循环就不会再碰它；
        `runner.drain()` 里的锁负责等已经在途的那一次发送收尾。
        """
        runner = self.runners.pop(bot_id, None)
        if runner is None:
            # 没有 runner 可排空（并发已经收走了）：租约交给扫描的「持有却没连接」分支去还，
            # 那里拿得到行上的代次。
            return
        try:
            await runner.drain()  # ①②
        ...
        await self.lease_loop.release(bot_id, runner.generation)
        self._log.info("bot_drained", bot_id=str(bot_id), generation=runner.generation)

    def note_fused(self, bot_id: uuid.UUID) -> None:
        """WS 踢线熔断：冷却期内不再认领，下一轮扫描把它交还出去。"""
        self._fused.add(bot_id)
        self.lease_loop.cool_down(bot_id, KICK_COOLDOWN_SECONDS)

    async def _release_fused(self, bot_id: uuid.UUID) -> None:
        self._fused.discard(bot_id)
        runner = self.runners.pop(bot_id, None)
        if runner is None:
            return
        await runner.stop()
        ...
        await self.lease_loop.release(bot_id, runner.generation, state="kicked")
    ...
    async def lease_connect(self, bot_id: uuid.UUID, generation: int) -> bool:
        ...
        if info is None:
            return False
        await self._start_runner(info, generation)
        return True

    async def lease_orphaned(self, bot_id: uuid.UUID) -> None:
        """租约已经不在本实例名下（被别人抢走、或被巡检释放），连接却还开着：立刻断掉。

        **不释放租约**：那一行现在是别人的，释放它等于把新持有者踢下台。只收自己这一半——
        WS 一断，企微那边就只剩新持有者的连接，不会两个实例互踢到熔断为止；这条流与出站箱
        此后归新持有者管（它们各自的代次校验会挡掉本实例的残余动作）。
        """
        runner = self.runners.pop(bot_id, None)
        if runner is None:
            return
    ...
    async def lease_maintain(self, held: list[BotLease]) -> None:
        """企微没有额外的逐轮维护：凭证变化走 `config_changed` 热更新。"""
    ...
    async def _lease_round(self) -> None:
        for bot_id in list(self._fused):
