...
async def _authorize_session_content(
    ...
    私聊归本人，不看角色：登录即可看自己的会话。会话历史里不能有别人的记录。对话记录在开流时
    就写入，第一轮也有记录可查；升级前的版本还没有这一行，它发出的私聊链接另带本人凭据
    （24 小时内、签给当前登录的人），升级窗口里旧版本开的第一轮靠它证明归属，所以仍然认。
    bot_token 登录一律不走本人这条路：群聊里的智能体拿着发言人的 token，不能借此读到他的私聊。
    非管理员还要节点已升级到记录不含环境变量的驱动。以本人身份运行过的定时任务与私聊同样只归执行者本人。
