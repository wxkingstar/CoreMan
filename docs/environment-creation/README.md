...
| workspace_root | 真实、可写、非符号链接的项目根目录；通过管理台「编辑运行时」修改，等待节点确认后生效，不手工改此键 |
...
### 11.5 修改项目主目录与 AI 可用状态

在「运行时管理 → 编辑运行时」修改项目主目录。节点必须在线且已升级到支持主目录编辑的 Runtime；已有 AI 员工绑定或正在迁入时，请先将它们切换到其他运行时。新路径必须是非根目录的绝对路径，不含 `..`，不能经过符号链接，且节点用户可写。

提交后显示「等待 Runtime 确认生效」。节点空闲时校验路径、创建缺失目录、持久保存配置并更新两个 AI 后端使用的主目录；服务端收到匹配回执后才展示新目录。在此之前不能给节点分配新员工。失败会保留原目录并显示原因，检查路径和权限后可重试。掉线时继续等待节点恢复，不能把等待状态当作成功；原目录中的文件不会自动搬迁或删除。

创建员工或切换运行时时，Claude Code / Codex 仅在已安装且登录状态为「已登录」时可选。「未安装」「未登录」「状态未知」会显示为禁用项，接口也会校验；状态未知时请升级 Runtime 并检查 CLI 登录。已有员工不会因这些提示被自动停用，同一实例上的模型调整不受新增绑定限制。

发布此功能时需先执行数据库迁移 `0036`，再部署服务端与前端；主目录编辑还需升级目标机器上的 Runtime。旧 Runtime 可继续心跳和运行任务，但不能远程修改主目录。
