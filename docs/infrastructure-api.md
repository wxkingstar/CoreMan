...
对话令牌使用 ES256，issuer 来自设置，aud/scope 为系统 key，主体为当前已验证且启用的发起者：sub **只取**其邮箱前缀（小写），业务系统按它对应自己的账号；同一前缀也以 `COREMAN_USER_SUBJECT` 下发给 CLI。没有邮箱、或另有账号的邮箱前缀相同时，平台**不签发任何令牌**，并在提示词中要求补全邮箱或联系管理员——绝不退回登录名：登录名与邮箱前缀共用一个命名空间，退回会让两个人拿到同一个 sub。授权测试的结果会写明所用的令牌用户。未知身份、bootstrap 身份、停用用户不签发。授权与机器人的允许范围每次请求重新读取，续跑也不复用历史提示词中的身份与令牌。公开公钥地址 `/api/.well-known/jwks.json`；轮换后旧公钥保留 24 小时。CoreMan bot_token 逐请求验证，不转换为寿命更长的管理会话。
