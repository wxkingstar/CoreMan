...
- Create a WeCom intelligent bot by scanning a QR code while creating an AI employee, the same way as for Feishu. The QR code is shown only to the person who started it and no link is offered, because its content alone is enough to fetch the bot secret. The server polls every 3 seconds, times the code out after 5 minutes on its own clock, keeps polling from the scheduler after the page is closed or the service restarts, drops its copy of the scan code as soon as the result arrives (WeCom keeps returning the same secret for that code afterwards, so a QR code that others may have seen calls for resetting the bot secret) and verifies the Bot ID and Secret with one `aibot_subscribe` on the long connection before the secret is stored encrypted and consumed once. An unused bot can be reused on the next attempt. This relies on the undocumented endpoints that the official WeCom CLI uses, so platform settings get a "Scan to create WeCom bots" switch; with it off, new WeCom employees need a manually entered Bot ID and Secret. Unrecognized responses are logged as `wecom_provision_unexpected_response` and raise an alert, and `coreman_wecom_provision_total{stage, result}` tracks generation, completion, expiry and verification. Requires database migration `0038`.
...
- Members can create their own scheduled AI tasks by asking a Feishu bot in a private chat, for example "每个工作日 9 点总结我的飞书未读消息". The assistant drafts the name, schedule and instruction with `schedule_propose`, and only the member's click on the confirmation card creates the task. Anyone allowed to use the bot can do this without being a bot administrator; each member can keep 10 active tasks per bot, recurring runs are at least an hour apart and one-off runs are between a minute and 30 days away. Tasks run as their owner and deliver only to the owner's private chat. The "我的提醒" page becomes "我的定时任务" and lists these tasks next to one-off reminders, with pause, resume, delete and the latest result. Bot administrators see them on the scheduled tasks page, marked as a member's own task, and can only disable them. Requires database migration `0037`.

### Changed

- Creating a WeCom AI employee with a manually entered Bot ID and Secret now verifies them with one long-connection subscription before saving, just like QR-created bots, and rejects credentials that WeCom refuses (HTTP 422) or cannot be checked (HTTP 502). Editing an existing employee's credentials is not verified, so the live connection is not kicked off.
- "Test as myself" on business systems no longer reports a failure just because the home page redirects after sign-in. It now requests the address once without the token and once with it: a 2xx, or a redirect to somewhere other than the unauthenticated destination and not a login page, counts as the token being recognized, and the message names the path it landed on. Redirecting to the same place or to a login page counts as not recognized, and a home page that opens without a token is reported as inconclusive. The response adds `baseline_status_code` and `redirect` (host and path only).
...
- Connecting Feishu now adds tools to the ordinary assistant instead of switching a private chat into a separate personal-data mode. After a member connects, their private chats with that bot keep the bot's prompt, skills, business system access, memory and sessions, and also get tools to read their own Feishu messages, meetings, minutes and documents (and to send messages at the top tier). The "普通助手" and "飞书资料" switch commands are gone and those words are ordinary messages again; group chats still never get the tools. These conversations are stored and resumed like any other private chat and only their owner can view them, replacing the memory-only sessions and context-bound links added earlier in this release. Scheduled tasks that their owner created, runs and receives alone may use the owner's authorization too; such runs are visible only to that person, including in chat logs and run history. Runtimes must report the new `feishu_personal_tools_v1` capability, which both the Claude and Codex drivers now do; the old `feishu_personal_restricted_v1` is no longer used, and a bot on an older runtime keeps answering without the personal tools. Requests like "每天……提醒我……" now reach the assistant, which can offer a scheduled task, instead of getting the one-off reminder help text.
...
- Installing a skill from a private repository that needs the source's access token failed with "操作失败（退出码 128）" on runtime nodes with Git older than 2.31. The token was passed through `GIT_CONFIG_COUNT`, which older Git silently ignores, so the clone was sent without credentials. The node now hands the token to Git through `GIT_ASKPASS`, as workspace Git operations already did, and passes the other settings with `git -c`, including an empty `http.proxy` so the token never goes through a proxy set in `~/.gitconfig`. Runtime nodes need an upgrade.
- The "可安装技能" heading in the skill management dialog no longer sits right under the installed skills table.
