...
export interface Source { id: string; key: string; label: string; git_url: string | null; categories: Record<string, string>; sort_order: number; version: number }
...
export interface Approval { skill_revision?: number; id: string; bot_id: string; skill_id: string; requested_databases: string[]; requested_security_prompt: string; approved_databases: string[] | null; status: string; version: number; requested_at: string; requested_by: string; reviewed_by: string | null; review_comment: string | null }
...
export const skills = {
  ...
  sourceSave: (row: Source | null, body: Omit<Source, 'id' | 'version'>) => call<Source>(row ? http.put(`${root}/skill-sources/${row.id}`, body, match(row.version)) : http.post(`${root}/skill-sources`, body)),
