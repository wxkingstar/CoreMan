...
it('keeps an existing expression on open and rewrites it only from the quick picker', async () => {
  vi.clearAllMocks()
  const wrapper = mount(CronEditorDialog, { props: { botOptions: [{ id: 'b1', name: '员工', platform: 'feishu' }] as never }, global: { plugins: [ElementPlus, i18n] } })
  await wrapper.vm.open({ id: 'j1', version: 3, bot_id: 'b1', name: '提醒', prompt: 'check', target_users: [], target_chats: [], notify_emails: [], schedule_kind: 'recurring', run_at: null, cron_expression: '0 9 * * 1,2,3,4,5', timezone: 'Asia/Shanghai', enabled: true } as never)
  await flushPromises()
  expect(wrapper.text()).toContain('每个工作日 09:00')
  await wrapper.get('[data-test="save-cron"]').trigger('click')
  await flushPromises()
  expect(cron.update).toHaveBeenLastCalledWith('j1', expect.objectContaining({ cron_expression: '0 9 * * 1,2,3,4,5' }), 3)
  wrapper.findAllComponents({ name: 'ElSelect' }).find(s => s.attributes('data-test') === 'cron-preset')!.vm.$emit('update:modelValue', 'daily')
  await flushPromises()
  expect(wrapper.text()).toContain('每天 09:00')
  await wrapper.get('[data-test="save-cron"]').trigger('click')
  await flushPromises()
  expect(cron.update).toHaveBeenLastCalledWith('j1', expect.objectContaining({ cron_expression: '0 9 * * *' }), 3)
  wrapper.unmount()
})

it('opens unrecognized expressions in custom mode', async () => {
  const wrapper = mount(CronEditorDialog, { props: { botOptions: [] }, global: { plugins: [ElementPlus, i18n] } })
  await wrapper.vm.open({ id: 'j2', version: 1, bot_id: 'b1', name: 'x', prompt: 'p', target_users: [], target_chats: [], notify_emails: [], schedule_kind: 'recurring', run_at: null, cron_expression: '0 8-18 * * 1-5', timezone: 'UTC', enabled: true } as never)
  await flushPromises()
  expect((wrapper.get('input[data-test="cron-expression"]').element as HTMLInputElement).value).toBe('0 8-18 * * 1-5')
  wrapper.unmount()
})
