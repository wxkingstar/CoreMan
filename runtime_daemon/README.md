...
- **技能安装失败**：管理台「技能管理」直接显示节点返回的原因，`runtime.log` 同时记录 `Operation install-skill failed: <原因>`（只有 Agent 的固定提示，不含命令输出与凭证）。「Git 来源不在白名单内」表示技能仓库的域名不在 `config.json` 的 `git_hosts` 中（管理台生成的安装链接只放行 `github.com`）：把主机名（如 `git.example.com`，不含协议与路径）加入该列表后重启服务。旧版节点只会报“未返回具体原因”，升级后才能看到节点侧原因。
